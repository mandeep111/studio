...
export async function getCounts() {
    ...
    const getCount = async (collectionName: string, qConstraints: any[] = []) => {
        try {
            ...
            // If collection doesn't exist or an index is missing, Firestore throws an error.
            // Gracefully handle this by returning 0.
            if (error.code === 'not-found' || error.code === 'failed-precondition') {
                 console.warn(`Collection '${collectionName}' not found or requires an index. Defaulting to 0 stats. This is normal for a fresh database.`);
                return 0;
            }
            // For other errors, log them so they can be debugged.
            console.error(`Error fetching count for ${collectionName}:`, error);
            // We throw the error so the page can be caught by the error boundary
            throw error;
        }
    };
    
    const [problems, solutions, ideas, businesses, investors] = await Promise.all([
        getCount("problems"),
        getCount("solutions"),
        getCount("ideas"),
        getCount("businesses"),
        getCount("users", [where("role", "==", "Investor")]),
    ]);

    return { problems, solutions, ideas, businesses, investors };
}
