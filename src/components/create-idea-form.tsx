...
const ideaFormSchema = z.object({
  title: z.string().min(5, { message: "Idea title must be at least 5 characters." }),
  description: z.string().min(20, { message: "Description must be at least 20 characters." }),
  price: z.string().optional(),
});

export default function CreateIdeaForm({ onIdeaCreated, isPaymentEnabled }: CreateIdeaFormProps) {
  ...
  const onSubmit = async (values: z.infer<typeof ideaFormSchema>) => {
    ...
    const result = await createIdeaAction(user.uid, formData);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="e.g., A subscription box for local artisans" {...field} disabled={isFieldsDisabled} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  placeholder="Describe your idea. What makes it unique? Who would benefit from it?"
        ...
        <div className="space-y-2">
            <Label>Funding Sought (Optional)</Label>
            {showPriceInput ? (
                <FormField
                  control={form.control}
                  name="price"
        ...
        <div className="space-y-2">
            <Label htmlFor="attachment-idea-form">Attachment (Optional)</Label>
            {!attachment ? (
                <Input
                    id="attachment-idea-form"
                    name="attachment"
                    type="file"
                    ref={fileInputRef}
        ...
        <div className="flex justify-end pt-4">
            <SubmitButton pendingText="Submitting..." disabled={isSubmitDisabled}>Submit Idea</SubmitButton>
        </div>
      </form>
    </Form>
  );
